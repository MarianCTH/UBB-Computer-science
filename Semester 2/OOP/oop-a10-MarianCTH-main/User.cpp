...
void MoviesWGUI::showUserMenu() {
	...
	ui.ExitForUser->show();
    ui.showTableButton->show();
    watchListModel = new MyModel();
    watchListModel->setObjectName("model");
    ui.watchListTable->setModel(watchListModel);
}
...
void MoviesWGUI::on_showTableButton_clicked() {
    this->hideSearchMovieForm();
    hideDeleteFromWatchListForm();
    ui.MoviesList->hide();
    this->hideUpdateMovieForm();
    ui.watchListTable->show();
    watchListModel->setDataInTable(this->controller.getMoviesFromWatchList());
}

...
Controller::Controller(Repository<Movie> movieRepository, Repository<Movie> watchlist) : repository(movieRepository), watchList(watchlist) {
}
...
void Controller::DeleteMovieFromWatchList(std::string title, int like) {
	...
	if (found_id != -1)
	{
		...
		Movie m = this->repository.getMovie(found_id);
